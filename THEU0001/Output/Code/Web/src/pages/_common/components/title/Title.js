...
class Title extends HTMLElement {
  ...
  __init(fCB) {
    LOG.info('~/pages/_common/components/title/Title :: __init');

    async.series([
      function (fCB) { this.createDomElements(fCB); }.bind(this),
      function (fCB) { this.createComponentInstances(fCB); }.bind(this),
    ], function (err, results) {
      LOG.info('~/pages/_common/components/title/Title :: __init (complete)');
  ...
  /// ANIMATE ///
  intro(fCB) {
    LOG.info('~/pages/_common/components/title/Title :: intro');
    ...
    async.parallel([
      ...
      LOG.info('~/pages/_common/components/title/Title :: intro (complete)');
      fCB();
  ...
  outro(fCB) {
    LOG.info('~/pages/_common/components/title/Title :: outro');
      ...
      this.oTweens['aTitleSplitOutro' + i] = TweenMax.to(this.oDOMElements['domTitleSplit' + i], 0.600,
        { css: { translateX: 0, opacity: 0.0 }, delay: (i*0.1), ease: Linear.easeNone, onComplete: function() {
          if (i === this.oDOMElements['domTitleWrapper'].children.length -1) {
            LOG.info('~/pages/_common/components/title/Title :: outro (complete)');
...
customElements.define('theu0001-pages-_common-components-title', Title);
