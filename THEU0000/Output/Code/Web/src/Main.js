...
import escapadefraktur_italic from './_assets/fonts/EskapadeFraktur-RegularItalic.woff2';
...
class Main {
  ...
  updateDOM(fCB) {
    const oCSSAssets = {
      sCSS: sCSS,
      fonts: {
        escapadefraktur_italic: { sPath: './assets/fonts/EskapadeFraktur-RegularItalic.woff2', sBuildPath: escapadefraktur_italic },
  ...
  createEventStreams(fCB) {
    ...
    /// WINDOW MOUSEMOVE ///
    FRP.createStream('window:onmousemove', { target: window, event: 'mousemove' });
    ...
    const _streamlistener1 = FRP.createStreamListener('window:onfocus', function () {
      if (process.env.NODE_ENV === 'production') {
        domIcon.setAttribute('href', '/static/icons/favicon-theundebruijn.png?' + process.env.BUILD_UUID);
        domAppleTouchIcon.setAttribute('href', '/static/icons/favicon-theundebruijn.png?' + process.env.BUILD_UUID);
      } else {
        domIcon.setAttribute('href', '/static/icons/favicon-theundebruijn.png');
        domAppleTouchIcon.setAttribute('href', '/static/icons/favicon-theundebruijn.png');
      };
    });
    ...
    const _streamlistener2 = FRP.createStreamListener('window:onblur', function () {
      if (process.env.NODE_ENV === 'production') {
        domIcon.setAttribute('href', '/static/icons/favicon-theundebruijn_inactive.png?' + process.env.BUILD_UUID);
        domAppleTouchIcon.setAttribute('href', '/static/icons/favicon-theundebruijn_inactive.png?' + process.env.BUILD_UUID);
      } else {
        domIcon.setAttribute('href', '/static/icons/favicon-theundebruijn_inactive.png');
        domAppleTouchIcon.setAttribute('href', '/static/icons/favicon-theundebruijn_inactive.png');
      };
    });
